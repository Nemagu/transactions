...
class RestorePersonalTransactionCommand:
    user_id: UUID
    transaction_id: UUID


class RestorePersonalTransactionUseCase(BaseUseCase):
    async def execute(
        self, command: RestorePersonalTransactionCommand
    ) -> PersonalTransactionSimpleDTO:
        action = "восстановление персональной транзакции"
        ...
        async with self._uow as uow:
            ...
            await uow.transaction_repositories.version.save(
                transaction, PersonalTransactionEvent.RESTORED, initiator.tenant_id
            )
